...
\begin{lstlisting}[language=C, caption=Grey Conversion Function, label={lst:gray_function}]
	void convertoToGrayScale(//omitting for space) {
        ...
        for (int i = 0; i < size; i++){
        		int chan_size = 3;
                auto r = uchar_image[chan_size * i];
                auto g = uchar_image[chan_size * i + 1];
                auto b = uchar_image[chan_size * i + 2];
...
Ultimately, we employ our final kernel, which could theoretically be executed within the first kernel. However, as we have seen that is not possible because certain things must be executed in order to run this last part of the algorithm. The code can be seen in Listing \ref{lst:last_func}.
